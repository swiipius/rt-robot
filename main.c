...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		printf("%d / %d / %d\n", digitalRead(LINE_LEFT), digitalRead(LINE_CENTER), digitalRead(LINE_RIGHT));
		// int var;
		// fflush(stdin);
		// scanf("%d", &var);
		// pwmWrite(PIN_BASE, angle(var + 10));

		start = clock();
		//+10 tkt la vcalibration
		dir = P_CONTROL(digitalRead(LINE_RIGHT), digitalRead(LINE_LEFT), digitalRead(LINE_CENTER));
		end = clock();
		if (dir > 0)
		{
			forward(100);
			...
			// printf("%d\n", dir);
			pwmWrite(PIN_BASE, angle(dir));
			delay(100);

			// cpu_time_used = ((double)(end - start)) / CLOCKS_PER_SEC;
			// printf("%f\n", cpu_time_used);
		}
		else if ((dir == -1) && (last_dir != 0))
		{
			printf("back\n");
			backward(100);
			if (last_dir == 1)
